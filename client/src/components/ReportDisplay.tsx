  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h3 className="text-xl font-bold text-gray-800 mb-1">{t('reportTitle')}</h3>
          <p className="text-sm text-gray-500">
            {t('createdOn')} {new Date(report.generatedAt || report.createdAt).toLocaleDateString(locale === 'ar' ? 'ar-SA' : 'en-US')}
          </p>
        </div>
        <button
      ...
      <div className="space-y-6">
        <div className="bg-gradient-to-r from-blue-50 to-cyan-50 border border-blue-200 rounded-xl p-4">
          <div className="flex items-center mb-2">
            ...
            <h4 className="font-semibold text-blue-800">{t('uploadFile')}</h4>
          </div>
        ...
        {report.prompt && (
          <div className="bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-200 rounded-xl p-4">
            <div className="flex items-center mb-2">
              ...
              <h4 className="font-semibold text-emerald-800">{t('prompt')}</h4>
            </div>
        ...
        <div className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl p-6 border border-gray-200">
          <div className="flex items-center mb-4">
            ...
            <h4 className="text-lg font-bold text-gray-800">{t('reportTitle')}</h4>
          </div>
        ...
        <div className="flex flex-wrap gap-4 text-sm text-gray-500">
          <div className="flex items-center">
            ...
            <span>{t('createdLabel')} {new Date(report.createdAt).toLocaleDateString(locale === 'ar' ? 'ar-SA' : 'en-US')}</span>
          </div>
          {report.generatedAt && (
            <div className="flex items-center">
              <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <span>{t('generatedLabel')} {new Date(report.generatedAt).toLocaleDateString(locale === 'ar' ? 'ar-SA' : 'en-US')}</span>
            </div>
          )}
        </div>
      </div>
